 ...
 ****************************************************************************
   ...
   SharkMQ (secure SMQ), Modified LED example.
   ...
   This code is the device side for the SMQ GPIO controller, an example
   that enables GPIO in a device to be controlled from a browser and
   Java (including Android).

   The code is designed for embedded devices, but can also be run on a
   host computer (Windows/Linux) as a simulated device
   ...
   * int setGPIO(int gpioId, int on);
   * int setGPIOFromDevice(int* gpioId, int* on);
   * int getGPIOState(int gpioId, int on, int set);
   * void setProgramStatus(ProgramStatus s);
   * int getUniqueId(const char** id); unless GETUNIQUEID is defined
   * xprintf (selib.h)

   See the following documentation for how to interface the GPIO
   functions to the hardware:
   https://realtimelogic.com/ba/doc/en/C/shark/md_md_Examples.html#LedDemo

   The above functions are documented in hardware-control.h. You may also study
...
/* Note that "proj-srv" should be defined in /etc/hosts to point to the
   server IP address on the local network. For a real life application
   should be be the DNS registered domain namd of the server.
 */
#define SIMPLEMQ_DOMAIN "http://proj-srv"
#define SIMPLEMQ_URL SIMPLEMQ_DOMAIN "/Server-Broker-Test8/"
...
/* The list of implemented GPIO pins in the device.
...
/* The GPIO controls: used by getGPIOState and setGPIO
 */
static int gpios[sizeof(machineInfo)/sizeof(machineInfo[1])];

/* Returns the GPIO on/off state for GPIO ID 'GPIOId'. The 'GPIOId'
...
int
getGPIOState(int GPIOId)
{
   baAssert(GPIOId >= 1 && GPIOId <= sizeof(machineInfo)/sizeof(machineInfo[1]));
   return gpios[GPIOId-1];
} 


/* Set GPIO on device. The 'GPIOId' is the 'handle' sent by the UI.
   Deliberately commented out to prevent crashes when run in a VirtualBox VM 
 */
 ...
 *
      if(GPIOId-1 == 0 && on == 0 && gpios[7] == 1)
      {
         gpios[7] = 0;
         digitalWrite(gpioPins[7],0);
         
      }
      if(GPIOId-1 == 0 && on == 0 && gpios[11] == 1)
      {
         gpios[11] = 0;
         digitalWrite(gpioPins[11],0);
         
      }
      if(GPIOId-1 == 0 && on == 0 && gpios[13] == 1)
      {
         gpios[13] = 0;
         digitalWrite(gpioPins[13],0);
...
/* Optional function that can be used to turn a GPIO pin on/off from the
   device by using buttons. The function must return TRUE if a button
   was pressed, otherwise FALSE must be returned. The GPIO state on/off
   information is managed by the online web service. 
*/
int
setGPIOFromDevice(int* gpioId, int* on)
{
   int gpioLen;
   const MachineInfo* machineInfo = getMachineInfo(&gpioLen);
   if(xkbhit())
   {
      int base,i;
      int c = xgetch();
   ...
   { /* Print out usage info at startup */
      ...
      if( ! oneshot )
      {
         oneshot = 1;
         xprintf(
            ("Set GPIO from keyboard. Uppercase = ON, lowercase = OFF.\n"
             "Switching GPIO state updates UI in all connected browsers.\n"));
      }
   }
   return 0;
}
...
/*
  The M2M-GPIO main function does not return unless the code cannot
...
*/
            if(msg[1] == -1)
            {
               xprintf(("ptid %X attempting to set invalid GPIO %d\n",
                        smq->ptid, msg[0]));
               return 0;
            }
      ...
      {
         int x; /* used for storing gpioId and temperature */
         int on;
         if(setGPIOFromDevice(&x,&on)) /* If a local button was pressed */
         {  /* Publish to all subscribed browsers and set the GPIO on/off */
            outData[0] = (U8)x; /* x is gpioId */
            outData[1] = (U8)on;
            /* Publish to "/m2m/led/device", sub-topic "led" */
            SharkMQ_publish(smq, outData, 2, deviceTid, ledSubTid);
            setGPIO(x, on); /* set the GPIO on/off */
         }
#ifdef ENABLE_TEMP
         x = getTemp();
