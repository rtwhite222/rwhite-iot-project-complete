 ...
 *   PROGRAM MODULE
 *
 *   $Id: m2m-led.c 3970 2017-02-02 20:02:10Z wini $
 *
...
//#define SIMPLEMQ_DOMAIN "http://172.20.20.112"
...
setLed(int ledId, int on)
{
   ...
   if(ledId >= 1 && ledId <= sizeof(ledInfo)/sizeof(ledInfo[1]))
   {
      ...
      digitalWrite(ledPins[ledId-1],on);
      if(ledId-1 == 7 && (leds[0] == 0 || (leds[1] == 0 && leds[2] == 0))){
         /*Sleep(5000);*/
         on = 0;
         digitalWrite(ledPins[ledId-1],on);
      }
      ...
      if(ledId-1 == 0 && on == 0 && leds[11] == 1)
      {
         leds[11] = 0;
         digitalWrite(ledPins[11],0);
         
      }
      if(ledId-1 == 0 && on == 0 && leds[13] == 1)
      {
         leds[13] = 0;
